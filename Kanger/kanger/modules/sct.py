...
async def msg_cmd(client, message):
    if not message.reply_to_message:
        return await message.reply(
            f"<b>Need Reply to user and Imput Text."
        )
            ...
            (
                InlineQueryResultArticle(
                    ...
                    input_message_content=InputTextMessageContent(
                        f"<b>New Secret Chat for U:</b> <a href=tg://user?id={m.reply_to_message.from_user.id}>{m.reply_to_message.from_user.first_name} {m.reply_to_message.from_user.last_name or ''}</a>"
                    ),
                )
            )
        ],
