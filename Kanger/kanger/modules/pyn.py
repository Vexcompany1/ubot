...
async def confirm_callback(client, callback_query):
    ...
    try:
        ...
        pesan = await bot.ask(
            user_id,
            f"<b>Sent our Payment Invoice: {full_name}</b>",
        ...
        if get.id in CONFIRM_PAYMENT:
            ...
            return await bot.send_message(get.id, "Aborted!")
    if get.id in CONFIRM_PAYMENT:
        if not pesan.photo:
            CONFIRM_PAYMENT.remove(get.id)
            ...
            buttons = [[InlineKeyboardButton("Confirm", callback_data="confirm")]]
            return await bot.send_message(
                user_id,
                """
<b>Cant be Processed!, Please Sent Real Invoice!</b>
""",
            ...
            return await bot.send_message(
                user_id,
                f"""
<b>Wait, Admin can Processing our Invoice.</b>
""",
            )
...
async def success_failed_home_callback(client, callback_query):
    ...
    if query[0] == "success":
        ...
        await bot.send_message(
            get_user.id,
            """
<b>Our Payment Has Been Accepted,\nU Now Can Generate Userbot</b>
""",
            reply_markup=InlineKeyboardMarkup(buttons),
        )
        buttons_success = [
            [
                InlineKeyboardButton(
                    "Get Profile", callback_data=f"profil {get_user.id}"
                )
            ],
        ]
        ...
        return await bot.send_message(
            OWNER_ID,
            f"""
<b>{get_user.first_name} {get_user.last_name or ''} Added to kangerPeople.</b>
""",
    ...
    if query[0] == "failed":
        buttons = [
            [
                InlineKeyboardButton(
                    "Order", callback_data="bayar_dulu"
                )
            ],
        ]
        await bot.send_message(
            get_user.id,
            """
<b>Your payment is rejected, please do it correctly</b>
""",
            reply_markup=InlineKeyboardMarkup(buttons),
        )
        buttons_failed = [
        ...
        return await bot.send_message(
            OWNER_ID,
            f"""
<b>{get_user.first_name} {get_user.last_name or ''} Not Added to kangerPeople</b>
""",
